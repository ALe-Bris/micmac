...
cAppli_YannScript::cAppli_YannScript(int argc, char ** argv){
	...
	GPSTime time = GPSTime(2021, 6, 1, 13, 30, 0);

	ObservationSlot slot = rov.lookForEpoch(time);
	std::vector<std::string> SATS = slot.getSatellites();

	std::cout << "----------------------------------------------------------" << std::endl;
	std::cout << slot.getTimestamp() << std::endl;
	std::cout << "----------------------------------------------------------" << std::endl;

    std::string pivot = SATS.at(0);
    for (unsigned i=1; i<SATS.size(); i++){
        std::string sat = SATS.at(i);
        std::cout << "DD "<< pivot << "-" << sat << "   " << -Algorithms::solve_ambiguity_ref(rov, bas, nav, time, pivot, sat) << std::endl;
    }

	/*
    std::cout << "DD G08-G10  " << Algorithms::solve_ambiguity_ref(rov, bas, nav, time, "G08", "G10") << std::endl;
    std::cout << "DD G08-G16  " << Algorithms::solve_ambiguity_ref(rov, bas, nav, time, "G08", "G16") << std::endl;
    std::cout << "DD G08-G18  " << Algorithms::solve_ambiguity_ref(rov, bas, nav, time, "G08", "G18") << std::endl;
	std::cout << "DD G08-G23  " << Algorithms::solve_ambiguity_ref(rov, bas, nav, time, "G08", "G23") << std::endl;
	std::cout << "DD G08-G26  " << Algorithms::solve_ambiguity_ref(rov, bas, nav, time, "G08", "G26") << std::endl;
    std::cout << "DD G08-G27  " << Algorithms::solve_ambiguity_ref(rov, bas, nav, time, "G08", "G27") << std::endl;
	std::cout << "----------------------------------------------------------" << std::endl;
    */
